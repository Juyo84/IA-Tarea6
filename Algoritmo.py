...
folder = 'C:\\Users\\HP\\Documents\\instancias\\'
datos = []
kmin = 1
...
def beneficio_peso(datos):
    ...
    for nodos in range(1, len(datos[0]), 1):
        ...
        valor_h.append(int(datos[1][nodos]) + int(datos[2][nodos]))

    return valor_h

def algoritmo(valor_h, datos):
    
    capacidad_Maxima = int(datos[3][0])     #   CAPACIDAD MAXIMA DEL PROBLEMA
    ...
    for mov_Nodos in range(0, iteraciones, 1):
        ...
        for iteracion in range (0, len(nodos),1):
            ...
            if(((int(datos[2][nodos[iteracion]]) + capacidad_Actual) <= capacidad_Maxima) and
        ...
        if(nodo_Aceptado != 0):
            ...
            beneficio += int(datos[1][nodo_Aceptado])
            capacidad_Actual += int(datos[2][nodo_Aceptado])
